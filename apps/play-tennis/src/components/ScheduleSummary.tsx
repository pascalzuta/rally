...
function getWeekOneMonday(tournament: Tournament): Date {
  // Use tournament.startsAt if available, otherwise fall back to createdAt's Monday
  if (tournament.startsAt) {
    // startsAt is already a Monday in YYYY-MM-DD format
    const [y, m, d] = tournament.startsAt.split('-').map(Number)
    const date = new Date(y, m - 1, d)
    date.setHours(0, 0, 0, 0)
    return date
  }
  // Fallback: find the Monday of the week the tournament was created
