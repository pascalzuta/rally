...
import { getPlayerName, getPlayerSeed, getAvailability, getPlayerRating, getCountyLeaderboard, getTournamentsByCounty, getIncomingOffers, getOutgoingOffers, getSeeds, winProbability, getRecentMatchResults, MatchResult } from '../store'
...
function InlineScoreEntry({ tournament, matchId, onSaved, onMatchResult }: {
  ...
  return (
    <>
      <button className="btn btn-primary" style={{ width: '100%' }} onClick={() => setShowScoreEntry(true)}>
        Enter Score
      </button>
      {showScoreEntry && (
...
export default function Home({
  ...
  // Recent matches
  const recentMatches = useMemo(() => getRecentMatchResults(profile.id, 5), [profile.id, tournaments])
      ...
      <div className="home-section home-section-spaced">
        ...
        {recentMatches.length > 0 && (
          <div className="card recent-matches">
            <h3 className="leaderboard-teaser-title">Recent Matches</h3>
            {recentMatches.map(rm => (
              <button
                key={rm.matchId}
                className="recent-match-row"
                onClick={() => onViewMatch(rm.tournamentId, rm.matchId)}
              >
                <span className={`match-result-indicator ${rm.won ? 'win' : 'loss'}`}>
                  {rm.won ? 'W' : 'L'}
                </span>
                <span className="match-opponent">vs {rm.opponentName}</span>
                <span className="match-score-compact">{rm.score}</span>
                {rm.ratingChange !== null && (
                  <span className={`match-rating-change ${rm.ratingChange >= 0 ? 'positive' : 'negative'}`}>
                    {rm.ratingChange >= 0 ? '+' : ''}{rm.ratingChange}
                  </span>
                )}
              </button>
            ))}
